 ...
 * Pure communication benchmark using staged-Isend/Irecv MPI:
 *   GPU -> host pinned buffer -> MPI_Irecv -> MPI_Isend -> host pinned buffer -> GPU
 *
 * Compile:
 *   nvcc -o ring_loop_staged_Isendrecv ring_loop_staged_Isendrecv.cu \
...
-np 2 ./ring_loop_staged_Isendrecv 262144 10 100
 */
...
int main(int argc, char **argv) {
    ...
    if(argc < 2){
        if(rank == 0)
            printf("Usage: mpirun -np N ./ring_loop_ISendRecv <kv_size_bytes> [warmup] [iters]\n");
        MPI_Finalize();
        return 1;
    }
    ...
    CHECK_CUDA(cudaMemset(d_buf_2, 0, kv_size));
    {
        ...
        for(int step = 0; step < size - 1; step++){
            ...
            if (h_recv[0] != expected * 1000.0f) {
                fprintf(stderr,
                        "[Rank %d] isendrecv precheck mismatch at step %d: got %.0f expect %.0f\n",
    ...
    if (rank == 0) {
        ...
        printf("RESULT backend=isendrecv bytes=%zu ranks=%d warmup=%d iters=%d "
