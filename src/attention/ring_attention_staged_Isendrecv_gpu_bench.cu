 ...
 * Ring Attention Skeleton — CUDA + MPI (staged-Isend/recv) + GPU attention
 *
 * Compile:
 *   nvcc -o ring_attention_staged_Isendrecv_gpu_bench ring_attention_staged_Isendrecv_gpu_bench.cu \
 ...
 *   -np 2 ./ring_attention_staged_Isendrecv_gpu_bench 262144 10 100
 */
...
int main(int argc, char **argv){
    ...
    if(rank == 0){
        printf("staged Isendrecv bench: warmup=%d iters=%d\n", warmup, iters);
