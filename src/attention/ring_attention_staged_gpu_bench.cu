 ...
 * Ring Attention benchmark — CUDA + MPI (staged)
 *
 * Compile:
nvcc -o ring_attention_staged_gpu_bench ring_attention_staged_gpu_bench.cu \
...
-np 2 ./ring_attention_staged_gpu_bench 262144 10 50
 */
#include<stdio.h>
#include<stdlib.h>
#include<math.h>
...
int main(int argc, char **argv){
    ...
    double sum_total = 0.0, sum_d2h = 0.0, sum_mpi = 0.0;
    double sum_h2d = 0.0, sum_attn = 0.0, sum_merge = 0.0;
    ...
    for(int iter = 0; iter < warmup + iters; iter++){
        ...
        double local_d2h = 0.0, local_mpi = 0.0, local_h2d = 0.0;
        double local_attn = 0.0, local_merge = 0.0, local_final = 0.0;
    ...
    if(rank == 0){
        printf("staged bench: warmup=%d iters=%d\n", warmup, iters);
