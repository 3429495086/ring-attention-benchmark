 ...
 * Ring Attention benchmark — staged MPI Isend/Irecv + GPU attention overlap
 *
 * Compile:
 *   nvcc -o ring_attention_staged_Isendrecv_overlap_gpu_bench \
 *   ring_attention_staged_Isendrecv_overlap_gpu_bench.cu \
 ...
 *   -np 2 ./ring_attention_staged_Isendrecv_overlap_gpu_bench 262144 10 100
 */
...
/*
 * GPU-side partial attention (placeholder for FlashAttention)
 *
 * Output:
 ...
 * acc_run is updated in-place. Division by l_run happens once after all steps.
 */
__global__
...
int main(int argc, char **argv){
    ...
    if(kv_size_ll <= 0 ||
       ...
       warmup < 0 || iters <= 0){
        if(rank == 0){
            printf("Invalid arguments: kv_size must be a positive multiple of %zu, "
                   "warmup >= 0, iters > 0.\n",
    ...
    printf("[Rank %d] seq_q=%d, seq_k=%d, dim=%d, KV shard=%.1f KB\n",
    ...
    for(int i = 0; i < seq_q * dim; i++)
        Q[i] = (rank * 500.0f + (float)(i % dim) * 0.01f) * 0.001f;
    ...
    for(int iter = 0; iter < warmup + iters; iter++){
        ...
        double local_d2h = 0.0;
        double local_mpi = 0.0;
        double local_h2d = 0.0;
        ...
        for(int step = 0; step < size - 1; step++){
            ...
            while(!event_ready(merge_end_event, rank) ||
                  !h2d_started ||
                  !event_ready(h2d_end_event, rank)){
        ...
        double local_vals[7] = {
            local_total, local_d2h, local_mpi, local_h2d,
            local_attn, local_merge, local_final
        };
        double max_vals[7];
        MPI_Reduce(local_vals, max_vals, 7, MPI_DOUBLE,
        ...
        if(rank == 0 && iter >= warmup){
            ...
            sum_h2d += max_vals[3];
            sum_attn += max_vals[4];
            sum_merge += max_vals[5];
            sum_final += max_vals[6];
        }
    }

    if(rank == 0){
        printf("staged Isendrecv OVERLAP bench: warmup=%d iters=%d\n",
               warmup, iters);
        printf("avg_total=%.3f ms avg_D2H=%.3f avg_MPI=%.3f avg_H2D=%.3f "
               "avg_attn=%.3f avg_merge=%.3f avg_final=%.3f\n",
               sum_total / iters, sum_d2h / iters, sum_mpi / iters,
               sum_h2d / iters, sum_attn / iters, sum_merge / iters,
               sum_final / iters);
    }
