#!/bin/bash
# benchmark_attention.sh - Launch Ring Attention benchmarks from a normal shell.
set -euo pipefail
...
if under_mpi_launcher; then
  echo "ERROR: benchmark_attention.sh should be run from a normal shell; it launches mpirun/srun itself." >&2
  exit 1
fi
